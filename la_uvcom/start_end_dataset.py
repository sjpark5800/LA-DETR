...
from la_uvcom.span_utils import span_xx_to_cxw
...
class StartEndDataset(Dataset):
                ...
                if "relevant_windows" in meta: ## For Qvhighlights test set
                    ...
                    if self.dset_name in ['charades', 'tacos', 'activitynet']:
                        ...
                        model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs[
                            "saliency_all_labels"] = \
                            self.get_saliency_labels_sub_as_query_(meta["relevant_windows"][0], meta["duration"], ctx_l)  # only one gt
    ...
    def get_saliency_labels_sub_as_query_(self, gt_window, duration, ctx_l, max_n=2):
        ...
        neg_pool = list(range(0, gt_st)) + list(range(gt_ed+1, ctx_l))
        if 'vgg' in self.dset_name or self.dset_name == 'nlq' or self.dset_name == 'hl':
            if len(neg_pool)==1:
    ...
    def _get_query_feat_by_qid(self, qid):
        ...
        # youtube-hl
            ...
            if 'llama' in self.q_feat_dir:
                q_feat_path = join(self.q_feat_dir, f"qid{qid}.pt")
                q_feat = torch.load(q_feat_path)
                q_feat = q_feat[:self.max_q_l]
                if self.normalize_t:
                    q_feat = l2_normalize_torch_tensor(q_feat)
                return q_feat
            else:
                # QVhighlight dataset
    ...
    def _get_video_feat_by_vid(self, vid):
        if self.dset_name == 'tvsum':
            ...
            if '6b' in self.v_feat_dirs[0]:
                _feat_dir = self.v_feat_dirs[0]
                _feat_path = join(_feat_dir, f"{vid}.pt")
                _feat = torch.load(_feat_path)[:self.max_v_l]
                if self.normalize_v:
                    _feat = l2_normalize_torch_tensor(_feat)
        ...
        if self.dset_name == 'tvsum':
            ...
            if '6b' in self.v_feat_dirs[0]:
                ...
                for vid, (s, e) in org_clip_ids_order:
                    ...
                    _feat_path = join(_feat_dir, f"{vid}.pt")
                    _feat = torch.load(_feat_path)[:self.max_v_l]
                    _feats.append(_feat[s:e])
                _feats = torch.cat(_feats, dim=0)
                
                if self.normalize_v:
                    _feat = l2_normalize_torch_tensor(_feats)
